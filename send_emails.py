...
empty = '<h2>К сожалению, на сегодня не нашлось ' \
        'профессий по Вашим предпочтениям.</h2>'

User = get_user_model()
qs = User.objects.filter(send_email=True).values(
    'city', 'speciality', 'email')
users_dct = {}
...
for keys in users_dct.keys():
    if keys not in urls_dct:
        if keys[0] and keys[1]:
            urls_err += f'<p"> Для города: {keys[0]} и ' \
                        f'ЯП: {keys[1]} отсутствуют ссылки</p><br>'
if urls_err:
